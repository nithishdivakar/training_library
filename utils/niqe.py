...
# params
model_mat = loadmat(DIR+'/modelparameters.mat')

MU_PRISPARAM = model_mat['mu_prisparam']
COV_PRISPARAM = model_mat['cov_prisparam']
...
def estimate_aggd_params(x):
    x_left  = x[x <  0]
    x_right = x[x >= 0]
    leftstd   = numpy.sqrt(numpy.mean(numpy.square(x_left)))
    rightstd  = numpy.sqrt(numpy.mean(numpy.square(x_right)))
 
    gam =  numpy.arange(0.2,10,0.001)
    r_gam =  ((gamma(2./gam))**2)/(gamma(1./gam)*gamma(3./gam));
    
    gammahat = leftstd/rightstd
    rhat = numpy.square(numpy.mean(numpy.abs(x)))/numpy.mean(numpy.square(x))
    rhatnorm  = (rhat*(numpy.power(gammahat,3)+1)*(gammahat+1))/(numpy.power((numpy.power(gammahat,2)+1),2))
    array_position = numpy.argmin(numpy.power((r_gam - rhatnorm),2))
    ...
    #if stddev_right == 0:
    #    return 1, 0, 0 # TODO check this
    #print(gammahat, leftstd,rightstd,rhat,rhatnorm,array_position, alpha, betal,betar)
  

    #alpha,betal,betar = 1,0,0
...
def niqe(img):
    ...
    model_mu = MU_PRISPARAM
    model_cov = COV_PRISPARAM
