...
class SRA_GANModel(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            # aesthetic feature
            if self.cri_aes:
                l_g_aes= self.l_aes_w * self.netA(self.fake_H, self.var_H)
                l_g_total += l_g_aes
        ...
        # RGAN loss
        if self.opt['train']['gan_type'] == 'vanilla':
            # real data
        ...
        elif self.opt['train']['gan_type'] == 'rgan':
            pred_d_real = self.netD(self.var_ref)
            pred_d_fake = self.netD(self.fake_H.detach())  # detach to avoid BP to G
 
            l_d_real = self.cri_gan(pred_d_real, True)
            
            l_d_real = self.cri_gan((pred_d_real - pred_d_fake), True)
            l_d_fake = self.cri_gan((pred_d_fake - pred_d_real), False)
            ...
            l_d_total += l_d_gp
             ...
             raise NotImplementedError(f"gan_type {self.opt['train']['gan_type']} not implemented")
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            if self.cri_aes:
                self.log_dict['l_g_aes'] = l_g_aes.item()
