...
def weights_init_kaiming(m, scale=1):
    ...
    if classname.find('Conv') != -1:
        try:
            ...
            logger.exception("weight initialisation failed for {}".format(classname),)
...
def define_A(opt):
    ...
    if which_model == 'nima':
        logger.info("Loading model for A (nima) [{:s}] ...".format(opt_net['chkp_path']))
        netE = arch.NIMALoss(model_path = opt_net['chkp_path'])
    else:
