...
class SREF_RGANModel(BaseModel):
    def __init__(self, opt):
        super(SREF_RGANModel, self).__init__(opt)
        train_opt = opt['train']
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            # aesthetic feature
            if self.cri_aes:
                l_g_aes= self.l_aes_w * self.netE(self.fake_H, self.var_H)
                l_g_total += l_g_aes
    ...
    def load(self):
        ...
        if self.opt['is_train'] and load_path_D is not None:
            ...
            self.load_network(load_path_D, self.netD,fastai_chkp=False)
